...
class TerminalStateless extends Component {
  ...
  _submitInput = (commandStr) => {
    ...
    //localStorage.setItem('fs', newState.getFileSystem(0));
  ...
  render() {
    const {
      acceptInput, autoFocus, clickToFocus, emulatorState, inputStr, theme, promptSymbol, outputRenderers, terminalId, cwd
    } = this.props;
    ...
    if (acceptInput) {
      inputControl = (
        <CommandInput
          ...
          value={inputStr}
          cwd={emulatorState.getEnvVariables().get("cwd")}
